...
public class Nonce {
    ...
    public static Nonce getInstance(){
        if(instance==null) instance=new Nonce();
        return instance;
    }
    
    public int getNonce(){
        if(userPrefs.getInt(NONCE_KEY, -1)==-1) setNonce(0); //если еще нет такого поля
    ...
    public void IncNonce(){       
        if(userPrefs.getInt(NONCE_KEY, 0)==Integer.MAX_VALUE) setNonce(0);
        else setNonce(userPrefs.getInt(NONCE_KEY, 0)+1);
    }
